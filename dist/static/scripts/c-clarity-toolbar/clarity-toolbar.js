...
;(function ($) {
  $.fn.moji_clarityToolbar = function () {
    ...
    if (document.location.search.indexOf('devtools=') >= 0) {
      ...
      var updateUrl = function updateUrl(param) {
      ...
      var showClass = function showClass(type) {
        // BUG: AF: Some elements are not showing the class for some reason. I don't have time to investigate at the moment.
        var elementList;
        if (type === 'c') {
          elementList = $("section[class*='c-']" || "section[class^=' c-'-']" || "header[class*='c-']" || "header[class^=' c-']" || "article[class*='c-']" || "article[class^=' c-']" || "nav[class*='c-']" || "nav[class^=' c-']" || "aside[class*='c-']" || "aside[class^=' c-']");
        } else {
      ...
      var getClassType = function getClassType(query) {
        switch (query) {
          case 'components':
            showClass('c');
            break;
